...
var pushfunctionR4 = function (value, quarter, selectedou) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var row = '<tr><td>Cv Statin</td><td>' + ounames[selectedou] + '</td><td>' + quarter + '</td><td>' + value + '</td><td>Success</td></tr>'
            $('.reporttable').append(row);
            totalPatientsWithCV = 0, totalPatientsWithCVonStatin = 0;
        },
        warning: function (response) {
            console.log("Warning! for OU = " + selectedou + " and Period = " + quarter);
            var row = '<tr><td>Cv Statin</td><td>' + ounames[selectedou] + '</td><td>' + quarter + '</td><td>' + value + '</td><td>Warning</td></tr>'
            $('.reporttable').append(row);
            totalPatientsWithCV = 0, totalPatientsWithCVonStatin = 0;
        },
        error: function (response) {
            console.log("ERROR for OU = " + selectedou + " and Period = " + quarter);
            var row = '<tr><td>Cv Statin</td><td>' + ounames[selectedou] + '</td><td>' + quarter + '</td><td>' + value + '</td><td>Error</td></tr>'
            $('.reporttable').append(row);
            totalPatientsWithCV = 0, totalPatientsWithCVonStatin = 0;
        }
    });
